    ...
    <header id="navbar">
        ...
        <nav>
            <ul class="nav-links">
                ...
                <?php if (isset($_SESSION['user_id'])): ?>
                    <li>
                        <a href="perfil.php" class="nav-cta">
                            <svg xmlns="http://www.w3.org/2003/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 5px;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                            Mi Perfil
                        </a>
                    </li>
                    <li>
                ...
                <?php else: ?>
                    <li><a href="login.php" class="nav-cta">Reservas</a></li>
                ...
                <form class="booking-form" action="registro.php" method="POST" id="bookingForm"
                    ...
                    <div class="form-group triple">
                        ...
                        <input type="email" name="email" placeholder="Correo Electrónico" 
                            ...
                            required>
                        <input type="tel" name="telefono" placeholder="WhatsApp (Ej: +123...)" 
                    ...
                    <div class="form-group">
                        ...
                        <select name="servicio" id="servicio-select" required
                            ...
                            <?php endforeach; ?>
                            <option value="reserva-domicilio">Reserva a Domicilio</option>
                        </select>
                    </div>
            ...
            <div class="footer-grid">
                ...
                <div class="footer-links">
                    ...
                    <ul>
                        ...
                        <li><a href="#booking" class="nav-cta" data-service="reserva-domicilio">Reservas a Domicilio</a></li>
                    </ul>
                </div>
                <div class="footer-contact">
                    <h4>CONTACTOS</h4>
                    <p><strong>Dirección:</strong> Los cocos de jacagua, Santiago los moreles</p>
                    <p><strong>WhatsApp:</strong> 8298949979</p>
                    <p><strong>Email:</strong> beautymakeup2312@gmail.com</p>
            ...
            document.querySelectorAll('.reserve-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (modal) {
                        ...
                        // Actualizar texto del botón de envío del modal
                        const submitBtnModal = modal.querySelector('button[type="submit"]');
                        if (submitBtnModal && optionToSelect) {
                            submitBtnModal.textContent = `Confirmar Reserva: ${optionToSelect.textContent}`;
                        }
                    }
            });
        });

        // Actualizar el botón si cambian la selección manualmente en el modal
            document.querySelector('select[name="servicio"]')?.addEventListener('change', (e) => {
                const sel = e.target;
                const opt = sel.options[sel.selectedIndex];
                const display = document.getElementById('selected-service-display');
                const submitBtnModal = document.querySelector('#booking-modal-overlay button[type="submit"]');
                if (display) display.textContent = `Servicio Seleccionado: ${opt.textContent}`;
                if (submitBtnModal) submitBtnModal.textContent = `Confirmar Reserva: ${opt.textContent}`;
            });
            ...
            // Enlace de 'Citas' en el navbar y footer
            document.querySelectorAll('.nav-cta').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (href === '#booking') {
                        ...
                        const modal = document.getElementById('booking-modal-overlay');
                        if (modal) {
                            // Pre-seleccionar servicio si existe data-service
                            const serviceToSelect = btn.getAttribute('data-service');
                            if (serviceToSelect) {
                                const sel = document.querySelector('select[name="servicio"]');
                                if (sel) {
                                    sel.value = serviceToSelect;
                                    const display = document.getElementById('selected-service-display');
                                    const submitBtnModal = modal.querySelector('button[type="submit"]');
                                    if (display) {
                                        const opt = Array.from(sel.options).find(o => o.value === serviceToSelect);
                                        const serviceName = opt ? opt.textContent : serviceToSelect;
                                        display.textContent = `Servicio Seleccionado: ${serviceName}`;
                                        if (submitBtnModal) {
                                            submitBtnModal.textContent = `Confirmar Reserva: ${serviceName}`;
                                        }
                                    }
                                }
                            }
                            ...
                            // Si no hay modal (usuario no logueado), redirigir a login
            ...
            // Alerta de cita para hoy
            <?php if (isset($_SESSION['user_id'])): ?>
                <?php
                $today = date('Y-m-d');
                $stmt_alert = $conexion->prepare("SELECT id FROM reservas WHERE cliente_id = ? AND fecha = ?");
                $stmt_alert->bind_param("is", $_SESSION['user_id'], $today);
                $stmt_alert->execute();
                $alert_res = $stmt_alert->get_result();
                if ($alert_res->num_rows > 0):
                ?>
                const alertModal = document.getElementById('success-modal');
                const alertMsg = document.getElementById('success-modal-msg');
                if (alertModal && alertMsg) {
                    alertMsg.textContent = "¡Recuerda que tienes una cita programada para el día de hoy!";
